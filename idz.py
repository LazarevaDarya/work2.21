...
def add_worker(
    ...
    cursor.execute(
        """
        INSERT INTO workers (worker_name, name_id, number, worker_year)
        VALUES (?, ?, ?, ?)
        """,
        (surname, name_id, number, year)
    )
...
def select_all(database_path: Path) -> t.List[t.Dict[str, t.Any]]:
    ...
    cursor.execute(
        """
        SELECT workers.worker_name, name.name_title, workers.number, 
        workers.worker_year
        FROM workers
...
def main(command_line=None):
    ...
    if args.command == "add":
        add_worker(db_path, args.surname, args.name, args.number, args.year)
